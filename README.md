...
```bash
npm run test:vision                     # 현재 버전 정확도 측정
npm run test:vision:repeat              # 동일 이미지 5회 반복 실행으로 재현성 확인
npm run test:vision -- --base=v1        # 베이스 프롬프트로 비교 측정
npm run test:vision -- --base=v3-calib1 # 재작성 프롬프트 + 초기 calibration으로 비교 측정
```

<br>

**버전별 개선 추이** (테스트 샘플 34장 기준)

| 단계      | 주요 변경                   | 통과  | 총 diff |
| --------- | --------------------------- | :---: | :-----: |
| v1        | 베이스 프롬프트             | 0/34  |   68    |
| v3-calib1 | 기본 프롬프트 재작성        | 8/34  |   37    |
| v16       | calibration 피드백 루프 누적 | 11/34 |   28    |

- 프롬프트를 버전 파일로 관리하고 **기본 프롬프트를 재작성**해 태그 오차(diff)를 68 → 37로 약 **46% 감소**시켰습니다. (calibration 조건은 동일하게 두고 기본 프롬프트만 교체: v1 → v3-calib1)
- AI 태깅에 대한 **사용자 수정을 프롬프트 개선에 반영하는 피드백 루프**를 구축해, 동일한 기본 프롬프트에서 calibration을 누적(v1 → v16)하며 태그 오차를 37 → 28로 약 **24% 추가 감소**시켰습니다.

<br>

**재현성 테스트** — `temperature: 0` 등 매개변수를 단일값으로 고정하고 동일 이미지를 5회씩 반복 실행한 결과, 34장 기준 약 **97%**의 태깅 일관성을 확보했습니다.

<br>

### AI 교정

태그를 수정할 때마다 원본·수정본 쌍을 corrections.json에 저장합니다. 수정이 5개 쌓일 때마다 자동으로 교정을 실행하며, 수동으로도 AI 학습 버튼으로 실행할 수 있습니다. 축적된 수정 이력을 Claude에 전달해 "어떤 사진에 어떤 태그를 달아야 하는지"에 대한 지침을 mood·colorTone·seasonFeel 카테고리별로 생성하고 calibration.json에 저장합니다. 이후 분석 시 이 지침이 프롬프트에 포함되어 개인 취향에 맞게 태그 정확도가 개선됩니다.

기존 지침은 카테고리별로 번호를 매겨 전달하고, AI는 카테고리마다 `수정`(기존 항목 교체)과 `추가`(새 패턴)만 응답하도록 했습니다. 언급되지 않은 항목은 그대로 유지되므로, 누적된 지침이 매 교정마다 통째로 재작성되지 않고 점진적으로 갱신됩니다. 전체 응답은 calibration_log.txt에 누적 기록해 AI가 어떤 이유로 지침을 도출했는지 추적할 수 있습니다.
